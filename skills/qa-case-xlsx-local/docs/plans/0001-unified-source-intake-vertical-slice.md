...
在 `D:\project\qa-case-xlsx` 中建立 `qa-case-xlsx-unified` 的首个可运行纵切：根据用户明确提供的定位符识别本地、飞书或混合来源，将各来源固化为可验证的 v2 `sources[]` 统一来源包，并在业务任务暂存区完成结构、哈希、修订、完整性和证据定位回读。
...
- 新建独立的 `qa-case-xlsx-unified` 入口，由本仓库持有并在仓库内复用用例生成核心的后续可复用部分。
...
- 任务根由系统工作区配置解析为 `<system-workspace>/tasks/<task_ref>/`。当前仓库开发环境使用已被 `.gitignore` 排除的 `D:\project\qa-case-xlsx\tasks/`；用户输入、来源内容和 Provider 返回值都不得决定物理任务路径。
...
- 迁移、复制或跨目录导入 `D:\project\work` 的 Provider 实现。
